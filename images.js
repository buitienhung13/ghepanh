...
const sections = [
  {
    ...
    items: [
      ...
      ["xhh17.jpg", 20000], ["xhh19.jpg", 25000], ["xhh21.jpg", 20000],
  ...
  { title: "Có nhận zalo: 0347510287 mình nha" }
];
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  sections.forEach(sec => {
    ...
    sec.items.forEach(([img, price], i) => {
      ...
      card.className = "card";
      card.dataset.originalPrice = price; // ✅ lưu giá gốc
      ...
      const priceDisplay = enableSale
        ? `<div class="overlay bottom">
            <s style="color:#a00;">${price.toLocaleString()} VNĐ</s>
            <span style="color:#000; font-weight:700;"> ${finalPrice.toLocaleString()} VNĐ</span>
          </div>`
        : `<div class="overlay bottom">Giá: ${price.toLocaleString()} VNĐ</div>`;
...
function updatePrices() {
  document.querySelectorAll(".card").forEach(card => {
    ...
    const originalPrice = parseInt(card.dataset.originalPrice);
    if (!isNaN(originalPrice)) {
      bottomOverlay.innerHTML = `Giá: ${originalPrice.toLocaleString()} VNĐ`;
    }
  });
}
